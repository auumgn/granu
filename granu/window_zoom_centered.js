...
function mytask() {
	...
	if (currentSize[0] != previousSize[0] || currentSize[1] != previousSize[1])
	{
		...
		topLeftX = ((currentSize[0] / finalZoom)- originalSize[0]) / 2;
		topLeftY = ((currentSize[1] / finalZoom)- originalSize[1]) / 2;
		
		this.patcher.message("script", "sendbox", "centeredUI", "patching_rect", topLeftX, topLeftY, originalSize[0], originalSize[1]);
	}
}
